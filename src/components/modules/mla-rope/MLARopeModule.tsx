// src/components/modules/mla-rope/MLARopeModule.tsx
...
import { Atom, MessageSquare, User } from 'lucide-react';
import { Dialog, DialogContent, DialogDescription, DialogFooter, DialogHeader, DialogTitle } from '@/components/ui/dialog';
import { ChatBot } from '@/components/ChatBot';

const MLARopeWorkflow = lazy(() => import('./components/MLARopeWorkflow').then(module => ({ default: module.MLARopeWorkflow })));

function MLARopeModule() {
    ...
    <div className={`h-screen w-full flex flex-col transition-colors duration-300 relative overflow-hidden ${isDark ? 'bg-slate-900 text-white' : 'bg-slate-100 text-slate-900'}`}>
        ...
        <DialogContent className="sm:max-w-lg bg-background/95 backdrop-blur-lg">
          <DialogHeader>
            ...
            <DialogDescription className="pt-2 text-base">
              Explore how modern Transformers inject positional data without sacrificing performance.
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            <p className="text-sm text-muted-foreground">
              This module demonstrates the full Multi-Head Latent Attention architecture from DeepSeek-V2. It adds **Decoupled Rotary Positional Embeddings (RoPE)** to the compressed attention mechanism.
            </p>
            <div className="mt-2 p-3 bg-indigo-500/10 border border-indigo-500/20 rounded-lg">
              <p className="text-indigo-400 font-medium">Your Mission:</p>
              <p className="text-sm mt-1 text-muted-foreground">
                Trace how separate "content" and "positional" vectors are created, rotated with RoPE, and then re-combined to form the final Query and Key vectors for attention.
              </p>
            </div>
          </div>
          <DialogFooter>
            <Button onClick={startWorkflow} className="w-full bg-gradient-to-r from-indigo-500 to-purple-500 text-white">
              Start Final Module
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <div className={`shrink-0 sticky top-4 mx-4 z-50 flex items-center justify-between backdrop-blur-md shadow-lg rounded-lg p-4 transition-colors duration-300 ${isDark ? 'bg-slate-800/70 border-slate-700/50' : 'bg-white/80 border-slate-300/60'}`}>
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-xl font-bold bg-gradient-to-r from-indigo-500 to-purple-500 bg-clip-text text-transparent cursor-pointer" onClick={() => navigate("/")}>
            MLA with Decoupled RoPE
          </h1>
        </div>
        <div className="flex items-center gap-4">
      ...
      <div className="flex-grow">
        <React.Suspense fallback={<div className="flex h-full w-full items-center justify-center">Loading Workflow...</div>}>
          <MLARopeWorkflow key={isWorkflowActive ? 'active' : 'inactive'} isActive={isWorkflowActive} />
        </React.Suspense>
      </div>
