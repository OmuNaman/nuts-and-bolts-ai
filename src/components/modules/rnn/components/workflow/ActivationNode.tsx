// src/components/modules/rnn/components/workflow/ActivationNode.tsx
...
export function ActivationNode({ data, id }: ActivationNodeProps) {
  ...
  // Find the index of the max value in each row (for softmax highlighting)
  const findMaxIndices = () => {
    if (!data.highlightMax || !isCompleted) return [];
    return data.expectedMatrix.map(row => {
      let maxIndex = 0;
      let maxValue = row[0];
  ...
  const maxIndices = findMaxIndices();
              ...
              return (
                <div 
                  ...
                  className={`text-xs font-mono px-2 py-1 rounded transition-colors ${
                    isMax && isCompleted
                      ? isDark 
                        ? 'bg-green-800/50 text-green-200 ring-1 ring-green-500/50'
                ...
                >
                  ...
                  {isCompleted && (
                    <span className="ml-1 font-sans">
                      {(data.expectedMatrix[0][i] * 100).toFixed(1)}%
                    </span>
                  )}
                </div>
              );
